...
// .wrangler/tmp/bundle-UpABeY/checked-fetch.js
...
// .wrangler/tmp/bundle-UpABeY/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-UpABeY/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
userRouter.post("/signup", async (c) => {
  ...
  try {
    ...
    const token = await sign2({ id: res.id }, c.env.secret);
...
blogRouter.use("/*", async (c, next) => {
  ...
  console.log(tkn);
  if (tkn) {
    const veriTkn = await verify2(tkn, c.env.secret);
    if (veriTkn) {
      c.set("id", veriTkn.id);
    } else {
